    ...
    return (
        <>
            <label className={`${errors ? 'text-red-500' : "text-qss-primary"} w-full flex justify-center flex-col gap-1 `}>
                {label}
                <div className={` ${errors ? "border-red-500 focus-within:border-red-500" : "border-gray-300 focus-within:bg-transparent"} group w-full border-qss-input bg-qss-background border rounded-3xl flex items-center gap-2 py-2 px-3 transition-all duration-900 `}>
                    ...
                    <input className={`${inputPassValue && !errors ? "text-green-600" : "text-qss-primary"} ${errors && "text-red-500 group-hover:text-red-500"} w-full border-0 text-lg bg-transparent  focus:outline-none`} type={showPassword ? "" : "password"} autoComplete='off' {...register} onBlur={handleErrors} />
