    ...
    return (
        <>
            <div className='flex flex-wrap gap-28 justify-center'>
                {
                    data.map((elem, i) => (
                        <div key={i} className={`group ${elem.type === 'Certificate' ? 'w-[169px]' : 'w-[136px]'}`}>
                            <div className='flex justify-between elems-center relative'>
                                <p className='group-hover:text-black text-[#7c7c7c] transition duration-[0.35s]'>{elem.type}</p>
                                {
                                    elem.type !== 'Report' && (
                                        <svg 
                            ...
                            <div className={`${elem.type === 'Certificate' ? 'h-[118px]' : 'h-[190px]'} group-hover:opacity-100 group-hover:border-qss-secondary overflow-hidden cursor-zoom-in transition duration-[0.35s] relative border-[1px] border-[#bcbcbc] opacity-60 mt-[10px] mb-8`}>
                                <img 
                            ...
                            <button className='w-full group-hover:bg-qss-secondary group-hover:text-white transition duration-[0.35s] text-xs py-[6.5px] px-[13px] rounded-[15px] cursor-pointer border-[1px] border-qss-secondary text-qss-secondary'>{checkButtonText(elem.type)}</button>
                        </div>
                    ))
                }
            </div>
